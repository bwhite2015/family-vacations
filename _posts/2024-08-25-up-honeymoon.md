---
# ===========================================================================
# COPY THIS WHOLE FILE to start a new entry.
#
# Save it as:  _posts/YYYY-MM-DD-some-short-title.md
# The date in the FILENAME sets the URL. The date in the FRONT MATTER below
# sets what's displayed and how entries are sorted. Keep them the same unless
# you have a reason not to.
# ===========================================================================

title: Upper Peninsula, MI - Honeymoon '24 - Day 8
# Displayed date AND sort order. Edit this freely — you can back-date an entry
# you're writing weeks later, or post-date one. Format:
#     YYYY-MM-DD HH:MM:SS ±HHMM
# The time is 24-hour. The last part is the UTC offset (-0600 = US Mountain in
# summer, -0700 in winter, -0500 US Central summer, -0800 US Pacific summer).
date: 2024-08-25 22:00:00 -0400
# Which vacation this belongs to. Must match a key in _data/trips.yml.
# Delete this line if the entry isn't part of a trip.
trip: up-honeymoon-2024
# The thumbnail in listings AND the big image at the top of the entry.
# Upload your photo to assets/images/ first, then point at it here.
image: /assets/images/up-honeymoon-2024/IMG_20240822_132159.jpg
image_alt: Aubri looking out towards Marquette near the summit of Sugarloaf Mountain.
image_caption: Great view of Marquette near the top of Sugarloaf Mountain.
author: Ben
# OPTIONAL extra photos, shown in a grid at the bottom of the entry.
# Delete this whole block if you don't want a gallery.
gallery:
  - src: /assets/images/up-honeymoon-2024/IMG_20240822_134505.jpg
    alt: Aubri taking the path back down from Sugarloaf Mountain. 
    caption: The path back down from Sugarloaf Mountain.
  - src: /assets/images/up-honeymoon-2024/IMG_20240822_154344.jpg
    alt: Aubri and I taking a selfie in front of the downtown Marquette building mural. 
    caption: Aubri and I in front of the Marquette building mural.
  - src: /assets/images/up-honeymoon-2024/IMG_20240822_151302.jpg
    alt: Pulled pork sandwich, a burger with bacon and goat cheese, pickled veggies with hummus, and a beer. 
    caption: Vanity shot of our food at the Iron Bay Tap Room.
  - src: /assets/images/up-honeymoon-2024/IMG_20240822_170157.jpg
    alt: The selection of meat inside Thill & Sons Fish House. 
    caption: The selection of meat inside Thill & Sons Fish House.
  - src: /assets/images/up-honeymoon-2024/IMG_20240822_173821.jpg
    alt: The view of Lake Superior from The Traverse on Presque Isle. 
    caption: At the edge of The Traverse and the abyss.


# OPTIONAL. Locations are normally picked up automatically from the tags you
# write in the body text below. Only use this if you want to pin the entry to a
# place you never actually name in the prose.
# locations:
#   - moab
---

##### *NOTE: The following blog entry – and really the entirety of this trip – was taken and digitized from a handwritten journal that had been used at the time to log Aubri and I's adventures. Because of this, stylistic differences in prose and writing style may be prominent when compared with more recent entries on this site. I've kept everything as accurate to the original entries as possible with only a few exceptions:*

- *I've added paragraph breaks to help ease eye strain.*
- *I've added images in between paragraphs as visual aids.*
- *I've adjusted common shorthand like  "+" to instead spell out the proper word, like "and" since there is ample room here (unlike in a 2" by 3" notebook).*



##### *Apart from these changes, you'll have to forgive the grammatical errors and fragmented sentences.*

___

Honeymoon - Day 8 - 8/25/24

___

Today, we drove 9 hours home from our honeymoon. The final day. I got us home in one piece. I'll continue my recitation below:

On Day 5, Aubri and I went to {% include loc.html id="up-marquette" %}. Our first stop was {% include loc.html id="up-sugarloaf" %}, a very short but uphill hike that left us somewhat winded but awestruck at the 360 degree view of the surrounding land. Stretching from East to West, the coastline of Northern UP butted up right against {% include loc.html id="up-lake-superior" %} and brought the forests to a halt. A remarkable sight to be sure. 

![The summit of Sugarloaf Mountain with Marquette in the distance.]({{ site.baseurl }}/assets/images/up-honeymoon-2024/IMG_20240822_133239.jpg)

Once we got into Downtown Marquette proper we walked the strip of local shops down to a pub called {% include loc.html id="up-iron-bay-tap-room" %}. Here we got a tasty meal of a pulled pork sandwich and a burger with bacon and goat cheese. These were both great, however the sides of pickled veggies with hummus overstayed their welcome for me. I also got the Saison Terre A Terre Farmhouse Ale by Barrel and Beam. It was decent, though, a small portion, which was fine since I was still trying to recover my microbiome in my stomach. 

![Downtown Marquette selfie in front of the Iron Bay Tap Room.]({{ site.baseurl }}/assets/images/up-honeymoon-2024/IMG_20240822_153215.jpg)

After this, exiting The Iron Bay Tap Room, I looked up to see {% include loc.html id="up-iron-ore-dock" %}, although I didn't know what it was until I bothered to ask an employee in the nearby establishment, {% include loc.html id="up-thill-sons" %}. Before leaving Marquette, we'd purchase 2 pounds of whitefish for $24. For now, we'd hold off until we could get some ice for them. We checked out some shops and got ice cream at {% include loc.html id="up-yoop-phoria" %} before walking down to the local sports store to get an idea of potential climbing locations around the area. Along the way, we checked out the local bookstore, also on {% include loc.html id="up-3rd-st-marquette" %}, {% include loc.html id="up-snowbound-books" %}. This victorian home was converted into a grand library of purchasable works. We didn't purchase any since I was still working on Walkter Isaacson's Benjamin Franklin. 

![The Iron Ore Dock next to Thill & Sons Fish House.]({{ site.baseurl }}/assets/images/up-honeymoon-2024/IMG_20240822_153457.jpg)

Once we got to the local sporting goods store, called {% include loc.html id="up-downwind-marquette" %}, We asked a man named Bill what he knew about local crags, which ended up being a lot. Bill gave us directions to {% include loc.html id="up-presque-isle" %} where a local boulder problem called {% include loc.html id="up-traverse" %} sat at cliff's edge facing Lake Superior just 10 feet away from the granite drop off. I worked the problem for a few hours while Aubri got some shots. Once satisfied with my progress, we picked up the fish, drove home and cooked em' up!

![Attempting The Traverse at Presque Isle.]({{ site.baseurl }}/assets/images/up-honeymoon-2024/climbing_stillframe_001.jpg)